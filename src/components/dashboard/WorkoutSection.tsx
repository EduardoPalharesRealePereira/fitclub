...
interface Exercise { name: string; sets: number; reps: string; rest: string; tip?: string; }
        ...
        <div style={{ padding: "0.75rem" }}>
          {today.exercises.map((ex, j) => (
            <div key={j} style={{
              ...
              border: "1px solid #1a2332", gap: "0.75rem",
            }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: "0.9375rem", display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.2rem" }}>
                  <span style={{
                    width: 22, height: 22, borderRadius: 6, background: `${color}20`,
                    color, fontSize: "0.6875rem", fontWeight: 900, display: "inline-flex",
              ...
              <div style={{ display: "flex", gap: "0.375rem", flexShrink: 0 }}>
                {[{ v: `${ex.sets}x`, l: "séries" }, { v: ex.reps, l: "reps" }, { v: ex.rest, l: "desc." }].map((s) => (
                  <div key={s.l} style={{ textAlign: "center", background: "#141e2e", borderRadius: 8, padding: "0.3rem 0.5rem", minWidth: 40 }}>
                    <div style={{ fontSize: "0.8125rem", fontWeight: 800, color }}>{s.v}</div>
